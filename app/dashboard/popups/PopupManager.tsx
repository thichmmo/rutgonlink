...
export default function PopupManager() {
  ...
  async function viewLinks(popup: Popup) {
    ...
    if (!response.ok) { setError('Không tải được link popup'); return }
    const details = await response.json()
    setViewing({ ...popup, settings: details.settings, firstUrl: details.firstUrl, secondUrl: details.secondUrl })
  }

  async function resolveAffiliate(platform: 'shopee' | 'tiktok') {
    ...
    try {
      ...
      if (platform === 'shopee') {
        ...
        setForm(current => ({ ...current, secondUrl: resolvedUrl }))
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Không xử lý được link') }
  ...
  return <div className="mx-auto max-w-7xl space-y-7">
    {viewing && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/60 p-3" role="dialog" aria-modal="true" aria-label="Liên kết popup"><div className="w-full max-w-xl space-y-4 rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-bold text-gray-950">{viewing.name}</h2><button onClick={() => setViewing(null)} className="rounded-lg px-3 py-1.5 text-sm text-gray-500 hover:bg-gray-100">Đóng</button></div>{[['Shopee', viewing.settings.shopee.url], ['TikTok Android', viewing.settings.tiktok.androidUrl], ['TikTok iOS', viewing.settings.tiktok.iosUrl]].map(([label, url]) => <div key={label} className="rounded-xl border border-gray-200 p-3"><p className="text-xs font-semibold uppercase text-gray-500">{label}</p><p className="mt-1 break-all text-sm text-gray-800">{url}</p><button onClick={() => void navigator.clipboard.writeText(url)} className="mt-2 text-xs font-semibold text-sky-700">Sao chép</button></div>)}</div></div>}
    ...
    {showForm && <form onSubmit={save} className="space-y-5 rounded-2xl border border-sky-200 bg-white p-5 shadow-sm sm:p-7"><div className="flex items-center justify-between"><h2 className="text-lg font-semibold text-gray-950">{editingId ? 'Sửa mẫu popup' : 'Tạo mẫu popup'}</h2><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.isActive} onChange={event => setForm({ ...form, isActive: event.target.checked })} className="h-4 w-4 accent-sky-600" /> Đang bật</label></div>
      ...
      <div className="grid gap-4 md:grid-cols-2"><fieldset className="space-y-3 rounded-xl border border-gray-200 p-4"><legend className="px-1 text-sm font-semibold text-gray-800">Lượt 1 · Shopee</legend><label className="grid gap-1 text-xs text-gray-600">URL<input required type="url" value={form.firstUrl} onChange={event => { const value = event.target.value; setForm(current => ({ ...current, firstUrl: value })); setSettings(current => ({ ...current, shopee: { ...current.shopee, url: value } })) }} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /></label><label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={settings.shopee.enabled} onChange={event => setSettings(current => ({ ...current, shopee: { ...current.shopee, enabled: event.target.checked } }))} /> Cho phép lượt này</label><div className="flex flex-wrap gap-4 text-xs"><label className="flex items-center gap-2"><input type="checkbox" checked={settings.shopee.androidEnabled} onChange={event => setSettings(current => ({ ...current, shopee: { ...current.shopee, androidEnabled: event.target.checked } }))} /> Android</label><label className="flex items-center gap-2"><input type="checkbox" checked={settings.shopee.iosEnabled} onChange={event => setSettings(current => ({ ...current, shopee: { ...current.shopee, iosEnabled: event.target.checked } }))} /> iOS</label></div><label className="grid gap-1 text-xs text-gray-600">Delay (giây)<input type="number" min="0" max="3600" value={settings.shopee.delaySeconds} onChange={event => setSettings(current => ({ ...current, shopee: { ...current.shopee, delaySeconds: Number(event.target.value) } }))} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /></label><input value={settings.shopee.imageUrl || ''} onChange={event => setSettings(current => ({ ...current, shopee: { ...current.shopee, imageUrl: event.target.value || null } }))} placeholder="Ảnh popup URL" className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /><button type="button" disabled={busy} onClick={() => void resolveAffiliate('shopee')} className="rounded-lg border border-sky-200 px-3 py-2 text-xs font-semibold text-sky-700">Xử lý link affiliate</button><input type="file" accept="image/*" onChange={event => void uploadImage(event.target.files?.[0], 'shopee')} className="text-xs" /></fieldset>
        <fieldset className="space-y-3 rounded-xl border border-gray-200 p-4"><legend className="px-1 text-sm font-semibold text-gray-800">Lượt 2 · TikTok</legend><label className="grid gap-1 text-xs text-gray-600">URL chung<input required type="url" value={form.secondUrl} onChange={event => { const value = event.target.value; setForm(current => ({ ...current, secondUrl: value })); setSettings(current => ({ ...current, tiktok: { ...current.tiktok, url: value, androidUrl: current.tiktok.androidUrl || value, iosUrl: current.tiktok.iosUrl || value } })) }} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /></label><label className="grid gap-1 text-xs text-gray-600">Android URL<input type="url" value={settings.tiktok.androidUrl} onChange={event => setSettings(current => ({ ...current, tiktok: { ...current.tiktok, androidUrl: event.target.value } }))} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /></label><label className="grid gap-1 text-xs text-gray-600">iOS URL<input type="url" value={settings.tiktok.iosUrl} onChange={event => setSettings(current => ({ ...current, tiktok: { ...current.tiktok, iosUrl: event.target.value } }))} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /></label><label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={settings.tiktok.enabled} onChange={event => setSettings(current => ({ ...current, tiktok: { ...current.tiktok, enabled: event.target.checked } }))} /> Cho phép lượt này</label><div className="flex flex-wrap gap-4 text-xs"><label className="flex items-center gap-2"><input type="checkbox" checked={settings.tiktok.androidEnabled} onChange={event => setSettings(current => ({ ...current, tiktok: { ...current.tiktok, androidEnabled: event.target.checked } }))} /> Android</label><label className="flex items-center gap-2"><input type="checkbox" checked={settings.tiktok.iosEnabled} onChange={event => setSettings(current => ({ ...current, tiktok: { ...current.tiktok, iosEnabled: event.target.checked } }))} /> iOS</label></div><label className="grid gap-1 text-xs text-gray-600">Chế độ iOS<select value={settings.tiktok.iosMode} onChange={event => setSettings(current => ({ ...current, tiktok: { ...current.tiktok, iosMode: event.target.value as PopupSettings['tiktok']['iosMode'] } }))} className="rounded-lg border border-gray-300 px-3 py-2 text-sm"><option value="desktop">Desktop</option><option value="onelink">OneLink</option></select></label><label className="grid gap-1 text-xs text-gray-600">Delay (giây)<input type="number" min="0" max="3600" value={settings.tiktok.delaySeconds} onChange={event => setSettings(current => ({ ...current, tiktok: { ...current.tiktok, delaySeconds: Number(event.target.value) } }))} className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /></label><input value={settings.tiktok.imageUrl || ''} onChange={event => setSettings(current => ({ ...current, tiktok: { ...current.tiktok, imageUrl: event.target.value || null } }))} placeholder="Ảnh popup URL" className="rounded-lg border border-gray-300 px-3 py-2 text-sm" /><button type="button" disabled={busy} onClick={() => void resolveAffiliate('tiktok')} className="rounded-lg border border-sky-200 px-3 py-2 text-xs font-semibold text-sky-700">Xử lý link affiliate</button><input type="file" accept="image/*" onChange={event => void uploadImage(event.target.files?.[0], 'tiktok')} className="text-xs" /></fieldset></div>
      ...
      <div className="grid gap-3 sm:grid-cols-2"><div className="relative flex aspect-video items-center justify-center rounded-xl bg-gray-950 bg-cover bg-center text-white" style={settings.shopee.imageUrl || form.imageUrl ? { backgroundImage: `url("${(settings.shopee.imageUrl || form.imageUrl).replaceAll('"', '%22')}")` } : undefined}><span className="rounded-full bg-red-600 px-4 py-2 text-sm font-semibold">▶ Shopee · lượt 1</span></div><div className="relative flex aspect-video items-center justify-center rounded-xl bg-gray-950 bg-cover bg-center text-white" style={settings.tiktok.imageUrl || form.imageUrl ? { backgroundImage: `url("${(settings.tiktok.imageUrl || form.imageUrl).replaceAll('"', '%22')}")` } : undefined}><span className="rounded-full bg-red-600 px-4 py-2 text-sm font-semibold">▶ TikTok · lượt 2</span></div></div>
